 ...
 * @author gmamakis
 */
public class LinkCheckingPlugin<I> extends
    ...
    public LinkCheckingPlugin(String name, String description) {
               ...
               /* String record = rdf.getProvidedCHOList().get(0).getAbout();
                ...
                for(String url:urls){
                    SourceDocumentReference docRef = new SourceDocumentReference(owner, url, null, null, 1l,
                            null);
                    docRefs.add(docRef);
                    tasks.add(new ProcessingJobTaskDocumentReference(DocumentReferenceTaskType.CHECK_LINK, docRef.getId()));
                }
    ...
    private Set<String> getUrls(RDF rdf) {
       Set<String> urls = new HashSet<>();
        ...
        if (aggr.getIsShownAt() != null) {
            String url = aggr.getIsShownAt().getResource();
            urls.add(url);
        }
