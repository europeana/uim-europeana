...
public class RetrievalServiceImpl extends AbstractOSGIRemoteServiceServlet implements
      ...
      BlockingInitializer mongoInitializer = new BlockingInitializer() {
        ...
        protected void initializeInternal() {
          try {
            ...
            mongoServer =
                new EdmMongoServerImpl(MongoProvider.getMongo(),
                    PropertyReader.getProperty(UimConfigurationProperty.MONGO_DB_EUROPEANA), uname,
                    pass);
  ...
  public MetaDataResultDTO getRecordsForCollection(String collectionId, int offset, int maxSize,
      String constraint) {
    StorageEngine<String> storage =
        (StorageEngine<String>) getEngine().getRegistry().getStorageEngine();
    if (storage == null) {
    ...
    if (recordId == null) {
      ...
      if (collection != null) {
        ...
        try {
          ...
          log.log(Level.WARNING, "Could not retrieve records for collection '" + collectionId
              + "'!", t);
          maxNumber = -1;
        }
        ...
        if (recordIds != null && recordIds.length > 0) {
          ...
          String[] subset =
              Arrays.copyOfRange(recordIds, Math.min(offset, recordIds.length - 1),
                  Math.min(offset + maxSize, recordIds.length));

          try {
            metaDataRecords = storage.getMetaDataRecords(Arrays.asList(subset));
          } catch (Throwable t) {
            log.log(Level.WARNING, "Could not retrieve records for Ids '" + Arrays.toString(subset)
                + "'!", t);
            maxNumber = -1;
          }
        }
    ...
    for (MetaDataRecord<String> metaDataRecord : metaDataRecords) {
      if (metaDataRecord != null && metaDataRecord.getId() != null) {
        ...
        try {
          ...
          String creationDate =
              metaDataRecord.getFirstValue(EuropeanaModelRegistry.UIMINGESTIONDATE);
          ...
          if (edmxml != null) {
            ...
            List<eu.europeana.corelib.definitions.jibx.EuropeanaType.Choice> dctypelist =
                cho.getChoiceList();
            if (dctypelist != null) {
    ...
    try {
      collection = storage.getCollection(collectionId);
      deleted_records =
          collection.getValue("Deleted") == null ? 0 : Integer.parseInt(collection
              .getValue("Deleted"));
    ...
    MetaDataResultDTO response =
        new MetaDataResultDTO(results, maxNumber, active_records, deleted_records);
  ...
  public EdmRecordDTO getSearchRecord(String recordId) {
    ...
    try {
      ...
      FullBeanImpl fullBean = (FullBeanImpl) mongoServer.getFullBean(recordId);
  ...
  private EdmFieldRecordDTO createMongoFiels(FullBeanImpl fullBean) {
    ...
    fieldValues.add(MongoConverter.getFieldValues(FullBeanImpl.class, fullBean, "getAbout",
        EdmLabel.EUROPEANA_ID));
    fieldValues.add(MongoConverter.getFieldValues(FullBeanImpl.class, fullBean, "getCountry",
        EdmLabel.COUNTRY));
    fieldValues.add(MongoConverter.getFieldValues(FullBeanImpl.class, fullBean,
        "getEuropeanaCollectionName", EdmLabel.EUROPEANA_COLLECTIONNAME));
    fieldValues.add(MongoConverter.getFieldValues(FullBeanImpl.class, fullBean, "getLanguage",
        EdmLabel.LANGUAGE));
    fieldValues.add(MongoConverter.getFieldValues(FullBeanImpl.class, fullBean, "getProvider",
        EdmLabel.PROVIDER));
    fieldValues.add(MongoConverter.getFieldValues(FullBeanImpl.class, fullBean, "getTitle",
        EdmLabel.TITLE));
    fieldValues.add(MongoConverter.getFieldValues(FullBeanImpl.class, fullBean, "getType",
        EdmLabel.TYPE));
    fieldValues.add(MongoConverter.getFieldValues(FullBeanImpl.class, fullBean, "getYear",
        EdmLabel.YEAR));
    ...
    if (aggregations != null) {
      for (AggregationImpl aggregation : aggregations) {
        fieldValues.addAll(MongoConverter.convertAggregation(aggregation));
        List<WebResourceImpl> webResources = (List<WebResourceImpl>) aggregation.getWebResources();
        if (webResources != null) {
          for (WebResourceImpl webResource : webResources) {
            fieldValues.addAll(MongoConverter.convertWebResource(webResource));
          }
        }
      }
    }

    EuropeanaAggregationImpl europeanaAggregation =
        (EuropeanaAggregationImpl) fullBean.getEuropeanaAggregation();
    if (europeanaAggregation != null) {
      fieldValues.addAll(MongoConverter.convertEuropeanaAggregation(europeanaAggregation));
      List<WebResourceImpl> webResources =
          (List<WebResourceImpl>) europeanaAggregation.getWebResources();
      if (webResources != null) {
        for (WebResourceImpl webResource : webResources) {
          fieldValues.addAll(MongoConverter.convertWebResource(webResource));
        }
  ...
  public LinksResultDTO getLinks(String recordId) {
    ...
    if (storage == null) {
      ...
      if (metaDataRecord == null) {
        ...
        links.add(new LinkDTO(PORTAL_SINGLE_RECORD_URL + "record" + recordId + ".html",
            "Record in portal"));

        links.add(new LinkDTO(PORTAL_PREVIEW_URL + "record" + recordId + ".html",
            "Preview Record in test portal"));

        links.add(new LinkDTO(REPOSITORY_PREVIEW_URL + "?recordID=" + uencoderecordId,
            "Preview cahched images in repository"));


        List<QualifiedValue<EuropeanaLink>> qualifiedValues =
            metaDataRecord.getQualifiedValues(EuropeanaModelRegistry.EUROPEANALINK);
  ...
  /**
   * Private method that determines the application context so that this does not have to be defined
   * in configuration properties. Ideally this should be put in the context of an overriden init()
   * method but unfortunately it does not work there.
   */
